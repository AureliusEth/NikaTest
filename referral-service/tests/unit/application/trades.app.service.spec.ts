...
class InMemTrades implements TradesRepository {
  public trades: any[] = [];
  async createTrade(id: string, userId: string, fee: number) {
    this.trades.push({ id, userId, fee });
  }
}
class InMemIdem implements IdempotencyStore {
...
class InMemRef implements ReferralRepository {
  ...
  async getAncestors(uid: string) {
    const a: string[] = [];
    let cur = this.map[uid];
    while (cur) {
      a.push(cur);
      cur = this.map[cur];
    }
...
describe('TradesAppService', () => {
  it('writes ledger entries for cashback and uplines and is idempotent', async () => {
    ...
    const users = new InMemUsers({
      U: { id: 'U', feeCashbackRate: 0.1 } as any,
    });
    ...
    const svc = new TradesAppService(
      trades as any,
      idem as any,
      ledger as any,
      ref as any,
