...
import { ReferralRepository } from '../../../../src/application/ports/repositories';
...
describe('ReferralService', () => {
  it('rejects self-referral', async () => {
    ...
    await expect(svc.computeLevelOrThrow('U', 'U')).rejects.toThrow(
      'Cannot self-refer',
    );
  });
  ...
  it('rejects when user already has referrer', async () => {
    const repo = new InMemoryReferralRepo([
      { referrerId: 'A', refereeId: 'U' },
    ]);
    const svc = new ReferralService(repo);
    await expect(svc.computeLevelOrThrow('U', 'B')).rejects.toThrow(
  ...
  it('detects cycles', async () => {
    ...
    const svc = new ReferralService(repo);
    await expect(svc.computeLevelOrThrow('U', 'A')).rejects.toThrow(
      'Cycle detected',
    );
  });
  ...
  it('computes level based on referrer depth and enforces ≤3', async () => {
    ...
    await expect(svc.computeLevelOrThrow('D', 'C')).rejects.toThrow(
