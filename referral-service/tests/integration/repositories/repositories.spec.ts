...
describe('Repository Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    referralRepo = module.get<PrismaReferralRepository>(
      PrismaReferralRepository,
    );
    ...
    idempotencyStore = module.get<PrismaIdempotencyStore>(
      PrismaIdempotencyStore,
    );
  });
  ...
  describe('PrismaUserRepository', () => {
    it('should create a new user when generating referral code for non-existent user', async () => {
      ...
      expect(code).toMatch(/^ref_[a-zA-Z0-9_-]+$/);
      ...
      expect(user?.email).toBe('TEST_USER_001@internal.nika.com');
    });
  ...
  describe('PrismaTradesRepository', () => {
    it('should create a trade', async () => {
      ...
      await tradesRepo.createTrade('TRADE_001', 'TRADER_001', 100.5);
      ...
      expect(Number(trade?.feeAmount)).toBe(100.5);
    });
  ...
  describe('PrismaLedgerRepository', () => {
    it('should record ledger entries', async () => {
      ...
      expect(Number(entries[0].rate)).toBe(0.3);
    });
