...
describe('EVM NikaTreasury Contract Integration', () => {
  ...
  beforeAll(async () => {
    if (skipTests) {
      ...
      console.log(
        '   2. Deploy contract: cd contracts/evm && forge script script/Deploy.s.sol --rpc-url http://localhost:8545 --broadcast',
      );
    ...
    if (!contractAddress) {
      ...
      throw new Error(
        'Please deploy contract first and set EVM_XP_CONTRACT_ADDRESS',
      );
    }
  ...
  describe('Root Storage and Retrieval', () => {
    (skipTests ? it.skip : it)(
      'should read current merkle root from contract',
      async () => {
    ...
    (skipTests ? it.skip : it)(
      'should update merkle root as owner',
      async () => {
    ...
    (skipTests ? it.skip : it)(
      'should emit RootUpdated event on update',
      async () => {
        // Arrange
        const oldRoot = await contract.merkleRoot();
  ...
  describe('Proof Verification - Single Leaf', () => {
    (skipTests ? it.skip : it)(
      'should verify valid proof for single-user tree',
      async () => {
    ...
    (skipTests ? it.skip : it)(
      'should reject invalid proof for single-user tree',
      async () => {
  ...
  describe('Proof Verification - Multi-Leaf Tree', () => {
    (skipTests ? it.skip : it)(
      'should verify valid proof in 2-user tree',
      async () => {
        ...
        const root = keccak256(
          Buffer.from(sortedLeaf1.slice(2) + sortedLeaf2.slice(2), 'hex'),
        );
        ...
        const isValidUser1 = await contract.verifyProof(
          proofForUser1,
          user1,
          token,
          amount1,
        ...
        const isValidUser2 = await contract.verifyProof(
          proofForUser2,
          user2,
          token,
          amount2,
    ...
    (skipTests ? it.skip : it)(
      'should reject proof with wrong sibling hash',
      async () => {
  ...
  describe('String Parameter Handling', () => {
    (skipTests ? it.skip : it)(
      'should handle email addresses as user IDs',
      async () => {
    ...
    (skipTests ? it.skip : it)(
      'should handle different tokens (USDC, XP, etc)',
      async () => {
        ...
        for (const test of tests) {
          const leafHash = keccak256(
            Buffer.from(`${userId}:${test.token}:${test.amount}`),
          );
          await (await contract.updateRoot(leafHash)).wait();
    ...
    (skipTests ? it.skip : it)(
      ...
      async () => {
        ...
        const leafHash = keccak256(
          Buffer.from(`${userId}:${token}:${correctAmount}`),
        );
        await (await contract.updateRoot(leafHash)).wait();
        ...
        const validResult = await contract.verifyProof(
          [],
          userId,
          token,
          correctAmount,
  ...
  describe('Gas Usage', () => {
    (skipTests ? it.skip : it)(
      'should have reasonable gas cost for root update',
      async () => {
        // Arrange
    ...
    (skipTests ? it.skip : it)(
      'should have reasonable gas for proof verification',
      async () => {
        // Arrange: Small proof
        const userId = 'user@example.com';
        const token = 'XP';
        ...
        const gasEstimate = await contract.verifyProof.estimateGas(
          proof,
          userId,
          token,
          amount,
