 ...
 * - We use 'as any' sparingly where Anchor's TypeScript types are incomplete
 */
@Injectable()
export class SvmBlockchainService {
    ...
    private initializeProgram(keypair: Keypair): void {
        ...
        const idl = NikaTreasuryIDL as Idl;
        ...
        try {
            ...
            this.program = new Program<NikaTreasury>(
                idl,
                this.provider
            );
    ...
    private getStateAddress(): string {
        ...
        if (!fs.existsSync(stateKeypairPath)) {
            throw new Error(
                `State keypair file not found at: ${stateKeypairPath}`
            );
        }
    ...
    async updateMerkleRoot(stateAddress: string, root: string): Promise<string> {
        ...
        try {
            ...
            const currentState = await (this.program.account as any).state.fetch(statePublicKey);
            ...
            const simulateResult = await (this.program.methods as any)
            ...
            const updatedState = await (this.program.account as any).state.fetch(statePublicKey);
    ...
    async getMerkleRoot(stateAddress: string): Promise<string> {
        ...
        try {
            ...
            const state = await (this.program.account as any).state.fetch(statePublicKey);
    ...
    async verifyProof(
        ...
        try {
            const sim = await (this.program.methods as any)
