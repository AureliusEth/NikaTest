...
│   - referral.app.service.ts                                 │
│   - trades.app.service.ts                                   │
└────────────────┬────────────────────────────────────────────┘
                 │ Uses
                 ▼
┌─────────────────────────────────────────────────────────────┐
...
│                                                              │
│  VALUE OBJECTS:                                              │
│   - Money        (wraps currency amounts)                    │
│   - Percentage   (wraps rates 0-1)                          │
│                                                              │
│  DOMAIN SERVICES:                                            │
│   - ReferralService     (validates referral rules)          │
│   - CommissionService   (calculates splits)                 │
└────────────────┬────────────────────────────────────────────┘
                 │ Implemented by
                 ▼
┌─────────────────────────────────────────────────────────────┐
...
│   - repositories/user.repository.prisma.ts                  │
│   - repositories/referral.repository.prisma.ts              │
│   - repositories/ledger.repository.prisma.ts                │
│   - repositories/trade.repository.prisma.ts                 │
│                                                              │
│  POLICIES (implement domain interface):                      │
│   - policies/default-policy.ts      (30%/3%/2%)            │
│   - policies/vip-policy.ts          (35%/5%/3%) [future]   │
│                                                              │
│  SERVICES:                                                   │
...
│   │   └── index.ts (CommissionPolicy interface)
│   │
│   ├── services/                    ← Domain Services
│   │   ├── referral.service.ts     (validates referral rules)
│   │   └── commission.service.ts   (uses policy for splits)
│   │
...
│   │   │
│   │   └── prisma.module.ts        (DI configuration)
│   │
│   └── policies/                    ← Policy IMPLEMENTATIONS
│       ├── default-policy.ts       (30%/3%/2%)
│       └── vip-policy.ts [future]  (35%/5%/3%)
│
